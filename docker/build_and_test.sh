...
CKPT_PATH="$PROJECT_DIR/wandb_logs/fubio/lkzjwmx6/checkpoints/fubio-epoch=007-val_mre_overall=3.941.ckpt"
...
# Copy checkpoint (if not already there or different)
if [ ! -f "$DOCKER_DIR/best_model.pth" ] || ! cmp -s "$CKPT_PATH" "$DOCKER_DIR/best_model.pth" 2>/dev/null; then
    echo "  Copying checkpoint (2.2GB, may take a moment)..."
    cp "$CKPT_PATH" "$DOCKER_DIR/best_model.pth"
else
    echo "  Checkpoint already in build context"
fi
...
echo "  Image built: $(docker images "$IMAGE_NAME" --format '{{.Repository}}:{{.Tag}} {{.Size}}')"
...
# Create {task_id}_test directories (symlinks to val data)
for task in A4C AOP FA FUGC HC IVC PLAX PSAX fetal_femur; do
    src="$PROJECT_DIR/data/val/$task"
    if [ -d "$src" ]; then
        ln -sfn "$src" "$INPUT_DIR/${task}_test"
        count=$(find "$src" -maxdepth 1 -type f 2>/dev/null | wc -l)
...
# --- Step 7: Compare with BS1 reference ---
echo ""
echo "[7/8] Comparing with BS1 reference..."

REF_DIR="$PROJECT_DIR/predictions/R48-EP7-TEACHER-TTA5-ELLIPSE-ENHANCE-BS1"
if [ ! -f "$REF_DIR/regression_predictions.json" ]; then
    echo "  WARNING: BS1 reference not found, skipping comparison"
else
    python3 -c "
import json, sys
...
echo ""
echo "  2. Record digest:"
echo "     docker inspect --format='{{index .RepoDigests 0}}' $FULL_IMAGE"
echo ""
echo "  3. Create submission.zip:"
echo "     mkdir -p /tmp/submission/submission"
echo "     echo '{\"image\": \"$FULL_IMAGE\"}' > /tmp/submission/submission/submission.json"
echo "     cd /tmp/submission && zip -r submission.zip submission/"
echo ""
echo "  4. Docker Hub: set repo to private, add 'lijiake' as collaborator"
echo ""
echo "  5. Upload submission.zip to CodaBench"
